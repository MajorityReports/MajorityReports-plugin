function Keyhandler(){
  ...
  handler.save = function save(scope){
    ...
    var temp_onkeyup = {};
    if(!scope){
      console.warn("keyhandler.save: No name given to scope!");
      return;
    }
    scopes[scope] = {};
  ...
  handler.clear = function clear(scope){
    if(scope){
      if(!scopes[scope]){
        console.warn("keyhandler.clear: '" + scope + "' is not a saved scope!");
        return;
      }
      delete scopes[scope];
    }
  ...
  handler.load = function load(scope){
    ...
    if(scope && !scopes[scope]){
      console.warn("keyhandler.load: '" + scope + "' is not a saved scope!");
      return;
    } else if(!scope){
      console.warn("keyhandler.load: No name given to scope!");
      return;
    }
    handler.clear();
